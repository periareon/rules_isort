...
Add `rules_isort` to your `MODULE.bazel` file

```python
bazel_dep(name = "rules_isort", version = "{version}")
```

Next, the `rules_isort` rules work mostly off of toolchains which are used to provide the necessary python targets (aka `isort`)
