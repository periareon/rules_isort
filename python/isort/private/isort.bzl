...
def _get_imports(target, aspect_ctx = None):
...
def _py_isort_target_aspect_impl(target, ctx):
    ...
    workspace_name = target.label.workspace_name
    if not workspace_name or workspace_name == ctx.workspace_name:
        imports = depset([ctx.workspace_name] + _get_imports(target, aspect_ctx = ctx))
    else:
        imports = depset()
